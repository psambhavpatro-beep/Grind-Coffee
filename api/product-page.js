...
// Fetches product from Firebase, returns full SEO HTML
...
async function findProductBySlug(slug) {
  ...
  for (const doc of snap.docs) {
    const data = doc.data();
    if (data.name && toSlug(data.name) === slug) {
...
<body>
  ...
  <div class="product-hero">
    <p class="breadcrumb"><a href="/">Home</a> → ${esc(product.name)}</p>
...
<head>
  ...
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@700&family=Inter:wght@400;600&display=swap');
    body { background: #f8f9f5; font-family: 'Inter', sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; text-align: center; }
    h1 { font-family: 'Libre Baskerville', serif; color: #1b4332; font-size: 36px; margin-bottom: 12px; }
...
<body>
  <div>
    ...
    <p>Sorry, we couldn't find that coffee. It may have been removed or the URL is incorrect.</p>
