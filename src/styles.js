...
export const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #f8f9f5; -webkit-tap-highlight-color: transparent; }
  ...
  .feat-card:hover { transform: translateY(-3px); box-shadow: 8px 8px 0 #1b4332 !important; }
  .feat:hover  { transform: translateY(-2px); box-shadow: 4px 4px 0 #111; }
  .ocard:hover { box-shadow: 4px 4px 0 #111; }
  input:focus, textarea:focus, select:focus { outline: none; border-color: #1b4332 !important; }
  ...
  @media (max-width: 640px) {
    ...
    .mobile-menu button { 
      text-align: left; padding: 13px 24px; font-size: 15px;
      width: 100%; background: none; border: none; cursor: pointer;
      font-family: sans-serif; color: #111; font-weight: 600;
  ...
  /* ── HERO ── */
  @media (max-width: 640px) {
    .hero-section { padding: 48px 18px 36px !important; }
    .hero-search-wrap { padding: 0 4px; }
  }

  /* ── FILTERS BAR ── */
  @media (max-width: 640px) {
    .filters-bar { padding: 14px 16px 10px !important; gap: 10px !important; }
    .filter-chips-row { display: flex; overflow-x: auto; -webkit-overflow-scrolling: touch; gap: 6px; padding-bottom: 4px; flex-wrap: nowrap !important; }
    .filter-chips-row::-webkit-scrollbar { display: none; }
  }

  /* ── DASHBOARD ── */
  @media (max-width: 640px) {
    .dash-wrap { padding: 16px 14px 60px !important; }
    .dash-tabs { gap: 0 !important; overflow-x: auto; flex-wrap: nowrap !important; }
    .dash-tabs::-webkit-scrollbar { display: none; }
    .dash-tabs button { flex-shrink: 0; font-size: 12px !important; padding: 10px 11px !important; }
    .stats-row { grid-template-columns: 1fr 1fr !important; }
    .vcard-row { flex-direction: column !important; align-items: flex-start !important; gap: 10px; }
    .vcard-btns { width: 100%; display: flex; gap: 8px; }
    .vcard-btns button { flex: 1; }
    .order-card-inner { flex-direction: column !important; align-items: flex-start !important; gap: 6px; }
  }

  /* ── TABLES (dashboard product table) ── */
  @media (max-width: 640px) {
    .dash-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; border-radius: 10px; }
    .dash-table-wrap::-webkit-scrollbar { height: 4px; }
    .dash-table-wrap::-webkit-scrollbar-thumb { background: #ccc; border-radius: 2px; }
  }

  /* ── PRODUCT PAGE ── */
  @media (max-width: 640px) {
    .prod-page { padding: 16px 14px 100px !important; }
    .prod-layout { gap: 20px !important; }
    .grind-grid { grid-template-columns: 1fr 1fr !important; }
    .sticky-cart-bar {
      position: fixed; bottom: 0; left: 0; right: 0;
      background: #fff; border-top: 2px solid #111;
      padding: 12px 16px; display: flex;
      align-items: center; justify-content: space-between;
      z-index: 90; box-shadow: 0 -4px 16px rgba(0,0,0,.1);
    }
    .sticky-cart-bar-hidden { display: none; }
    .price-cta-desktop { display: none !important; }
  }
  ...
  /* ── CART DRAWER ── */
  @media (max-width: 640px) {
    .drawer-inner { width: 100% !important; max-width: 100vw !important; }
  }

  /* ── MODALS ── */
  @media (max-width: 640px) {
    .modal-box { padding: 24px 18px !important; margin: 0 !important; border-radius: 16px 16px 0 0 !important; position: fixed !important; bottom: 0 !important; top: auto !important; max-height: 92vh !important; border-left: 2px solid #111 !important; border-right: 2px solid #111 !important; border-bottom: none !important; }
    .modal-overlay { align-items: flex-end !important; padding: 0 !important; }
  }

  /* ── EXPLORE ROASTERS ── */
  @media (max-width: 640px) {
    .roaster-grid { grid-template-columns: 1fr !important; padding: 16px 14px 40px !important; }
    .feat-roaster { flex-direction: column !important; }
    .feat-roaster img { width: 100% !important; height: 200px !important; object-fit: cover; }
  }
`;
...
export const S = {
  ...
  featBadge: { position: "absolute", top: 10, left: 10, background: "#1b4332", color: "#fff", fontSize: 9, padding: "3px 9px", borderRadius: 20, fontFamily: "sans-serif", fontWeight: 700, letterSpacing: .5 },
  cardBody: { padding: "14px 18px 18px" },
  cardOrig: { color: "#1b4332", fontSize: 9, letterSpacing: 3, textTransform: "uppercase", marginBottom: 3, fontFamily: "sans-serif", fontWeight: 700 },
  etaBadge: { display: "inline-flex", alignItems: "center", gap: 4, background: "#eaf4e8", border: "1px solid #b7d9b1", borderRadius: 20, color: "#1b4332", fontSize: 10, padding: "2px 8px", fontFamily: "sans-serif", fontWeight: 700, marginBottom: 7 },
  ...
  roasterTag: { background: "#eaf4e8", border: "1px solid #b7d9b1", borderRadius: 20, color: "#1b4332", fontSize: 11, padding: "3px 10px", cursor: "pointer", marginBottom: 10, fontFamily: "sans-serif", fontWeight: 600, display: "inline-block" },
  ...
  viewBtn: { background: "none", border: "none", color: "#1b4332", cursor: "pointer", fontSize: 13, fontWeight: 700 },

  // PRODUCT PAGE
  prodPage: { maxWidth: 1060, margin: "0 auto", padding: "28px 26px 60px", background: "#f8f9f5" },
  backBtn: { background: "none", border: "none", color: "#1b4332", cursor: "pointer", fontSize: 13, marginBottom: 22, padding: 0, fontFamily: "sans-serif", fontWeight: 700 },
  ...
  reviewBig: { fontSize: 36, fontWeight: 700, color: "#1b4332" },
  badgePill: { background: "#eaf4e8", color: "#2d6a4f", padding: "4px 12px", borderRadius: 20, fontSize: 11, fontFamily: "sans-serif", fontWeight: 600, border: "1px solid #b7d9b1" },
  prodOrig: { color: "#1b4332", fontSize: 9, letterSpacing: 4, textTransform: "uppercase", marginBottom: 8, fontFamily: "sans-serif", fontWeight: 700 },
  prodTitle: { fontSize: 32, fontWeight: 700, lineHeight: 1.15, marginBottom: 14, color: "#111" },
  roasterLink: { display: "flex", alignItems: "center", gap: 12, background: "#eaf4e8", border: "2px solid #b7d9b1", borderRadius: 10, padding: "10px 14px", marginBottom: 18, cursor: "pointer", width: "fit-content" },
  ...
  grindOn: { borderColor: "#1b4332", background: "#eaf4e8", color: "#2d6a4f" },
  ...
  infoNote: { color: "#1b4332", fontSize: 11, marginBottom: 14, fontStyle: "italic", fontFamily: "sans-serif" },
  ...
  tabOn: { color: "#1b4332", borderBottomColor: "#1b4332" },
  ...
  statVal: { fontSize: 24, fontWeight: 700, color: "#1b4332", marginBottom: 2 },
  ...
  stepDotOn: { background: "#1b4332", borderColor: "#1b4332", color: "#fff" },
  stepLine: { flex: 1, height: 2, background: "#e5e5e5", margin: "13px 0 0" },
  stepLineOn: { background: "#1b4332" },
